    ...
    <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-200 overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                    ...
                    <td class="px-5 py-3.5 text-right">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <form method="POST" action="{{ route('admin.rooms.destroy', $room) }}"
                                  onsubmit='return confirm("Delete " + @json($room->name) + "? This cannot be undone.")'>
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-5 py-12 text-center text-sm text-gray-400">No rooms yet. <a href="{{ route('admin.rooms.create') }}" class="text-[#7C0E52] underline">Add one.</a></td>
                </tr>
                @endforelse
            </tbody>
        </table>
